...
function _artistpublic_columnKey(artistver){return(
Object.keys(artistver[0])[4]
)}

function _artistpublic_Column(artistver,artistpublic_columnKey){return(
artistver.map(row => String(row[artistpublic_columnKey]))
)}
...
Plot.plot({
  ...
  color: {
    ...
    range: ['#FF0000', '#FFD700'],  // 你可以根據需要更改顏色
    legend: true
  },
  marks: [
  ...
  series.forEach((serie) => {
      ...
      bars.enter().append("rect")
          ...
          .on("mouseover", function(d) {
              d3.select(this).attr("fill", "#7FFF00");
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("artistpublic_columnKey")).define("artistpublic_columnKey", ["artistver"], _artistpublic_columnKey);
  main.variable(observer("artistpublic_Column")).define("artistpublic_Column", ["artistver","artistpublic_columnKey"], _artistpublic_Column);
