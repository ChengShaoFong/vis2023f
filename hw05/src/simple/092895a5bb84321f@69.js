...
function _simple1(d3,simple_data,invalidation)
{
  ...
  const simulation = d3.forceSimulation(nodes)
      .force("link", d3.forceLink(links).id(d => d.id).distance(25).strength(1))
 ...
 // 設定節點初始位置在畫布的中間
  ...
  // 更新力模擬的y方向力，讓節點向下運動
  simulation.force("y", d3.forceY().strength(0.1).y(d => d.depth * 30)); // 調整力的方向和大小
