import define1 from "./a601aba88046a626@350.js";
import define2 from "./0edfeb766ced5a8c@382.js";

function _artist(FileAttachment){return(
FileAttachment("artist-1.csv").csv()
)}

function _2(__query,artist,invalidation){return(
  ...
  svg
    ...
    .on("mouseenter", (event, d) => {
      ...
      label
        ...
        .text(d.data.name);
      const percentage = ((36 * d.value) / root.value).toPrecision(3);
      label
        .style("visibility", null)
        .select(".percentage")
        .text(percentage + "%");
...
<h3>從上方的旭日圖中，我們可以看出：
  <ul>
    <li>替代空間工作地點的人，依比例來說有較多的減碳行為</li>
    <li>其中工作室的人在問卷中佔大宗，超過一半的人有減碳行動</li>
    <li>非營利組織也是，該問卷明顯的分布不同工作地點做的減碳方式</li>
  </ul>
</h3>`
)}
...
function _partition1(d3,radius){return(
data =>
  d3.partition().size([2 * Math.PI, radius * radius])(
    d3
      .hierarchy(data)
...
d3
  ...
  .range(["#FFD700","#FF0000","#DC9FB4","#F08080","#DC143C","#CD5C5C"])
  .unknown("#BEBEBE")
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["__query","artist","invalidation"], _2);
  ...
  main.variable(observer("partition1")).define("partition1", ["d3","radius"], _partition1);
  ...
  main.import("Sunburst", child1);
  const child2 = runtime.module(define2);
  main.import("Pack", child2);
  return main;
}
